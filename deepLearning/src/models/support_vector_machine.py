...
def write_svm_csv(acc, metric, out_path, lock=None, metric_name='contrast'):
    if lock is not None:
        lock.acquire()
    ...
    with open(svm_csv, 'a') as csv_file:
            headers = ['svm_accuracy', metric_name]
            writer = csv.DictWriter(csv_file, delimiter=';', lineterminator='\n',fieldnames=headers)
            if not file_exists:
                writer.writeheader()  # file doesn't exist yet, write a header
            writer.writerow({'svm_accuracy': acc, metric_name: metric})
    if lock is not None:
        lock.release()


def get_svm_accuracy(path_mat, num_samples=15000):
    start = time.time()
    meanData, meanDataLabels, dataContrast = get_h5mean_data(path_mat, includeContrast=True)
    ...
    print(f'Accuracy is {acc}, num train samples is {num_train}, took {str(datetime.timedelta(seconds=time.time()-start))}.')
    return acc, float(dataContrast[0])
...
if __name__ == '__main__':
    ...
    for num in sample_numbers:
        get_svm_accuracy(path_mat, num_samples=num)
