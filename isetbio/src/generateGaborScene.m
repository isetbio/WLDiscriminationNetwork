...
defaultStimParams = struct(...
    ...
    'pixelsAlongHeightDim', 256, ...        % pixels- height dimension    'pixelsAlongWidthDim', 256, ...         % pixels- width dimension
    'signalGridSize', 1, ...                % grid size, where signal location can be specified (2x2 grid -> 2)
    'signalLocation', [1], ...              % location, where to put the signal. In a 2x2 grid: 1 -> upperLeft, 2 -> upperRight, 3 -> lowerRight etc. etc.
    'gridZoom', 1 ...                % zoom the grid into the display. Useful, if some extra padding around is needed to include eye movement..
    );
%% parse input
p = inputParser;
...
imageHarmonicParams = struct(...
        ...
        'signalGridSize', stimParams.signalGridSize, ...
        'signalLocation', stimParams.signalLocation, ...
        'gridZoom', stimParams.gridZoom);
